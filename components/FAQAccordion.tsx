...
const faqs = [
  ...
  { q: "추천 비료는 어디서 주문하나요?", a: "추천 결과에서 장바구니 담기나 바로 주문하기를 선택하거나 비료마켓에서 같은 상품을 찾아 주문할 수 있습니다." }
];
    ...
    <div className="mx-auto max-w-3xl space-y-3">
      {faqs.map((faq, index) => (
        <article key={faq.q} className="soft-card overflow-hidden">
          <button aria-label={`${faq.q} 답변 열기`} onClick={() => setOpen(open === index ? -1 : index)} className="flex w-full items-center justify-between p-5 text-left font-black text-[#173f22]">
            {faq.q}
